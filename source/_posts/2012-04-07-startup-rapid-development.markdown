...
不可否認有些開發者效能和 Fancy 技術實在追求過頭，比如說甚至一開始就用 Backbone 寫整個網站，或者是從頭到尾使用 Nodejs 寫網站。這都是一開始就打算寫 mobile 版 web service 給 mobile phone 使用才需要做的事。因為 3G 的 Latency 實在太大，要盡力的壓縮頻寬使用量和追求頁面 response time。
